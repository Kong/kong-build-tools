...
docker run ${USE_TTY} --user=root --rm ${KONG_TEST_IMAGE_NAME} /bin/sh -c "/usr/local/openresty/bin/openresty -V 2>&1 | grep pcre"
...
if [[ "$SSL_PROVIDER" = "openssl" ]] && [[ ! -z "${RESTY_OPENSSL_VERSION:-}" ]]; then
