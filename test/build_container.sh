...
if [ "$RESTY_IMAGE_BASE" == "alpine" ]; then
  ...
  cp output/linux-amd64/${KONG_PACKAGE_NAME}-${KONG_VERSION}.el${RESTY_IMAGE_TAG}.amd64.rpm output/kong.rpm
elif [ "$RESTY_IMAGE_BASE" == "amazonlinux" ]; then
  DOCKER_FILE="Dockerfile.rpm"
  cp output/linux-amd64/${KONG_PACKAGE_NAME}-${KONG_VERSION}.aws.amd64.rpm output/kong.rpm
elif [ "$RESTY_IMAGE_BASE" == "rhel" ]; then
	cp output/linux-amd64/${KONG_PACKAGE_NAME}-${KONG_VERSION}.rhel${RESTY_IMAGE_TAG}.amd64.rpm output/kong.rpm
