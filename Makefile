...
ANCHORE_ACTION_TYPE?=analyze-image
...
ANCHORE_SBOM_FILE_NAME?=
ANCHORE_APPLICATION_DIR=$(KONG_SOURCE_LOCATION)
...
scan-kong: 
ifeq ($(ANCHORE_ACTION_TYPE),analyze-image)
	...
	ANCHORE_SBOM_FILE_NAME=${ANCHORE_SBOM_FILE_NAME} \
	ANCHORE_OUTPUT=${ANCHORE_OUTPUT} \
	ANCHORE_APPLICATION_DIR=${ANCHORE_APPLICATION_DIR} \
	./scan-kong.sh
else ifeq ($(ANCHORE_ACTION_TYPE),analyze-source)
	ANCHORE_ACTION_TYPE=$(ANCHORE_ACTION_TYPE) \
