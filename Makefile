...
ifeq ($(RESTY_IMAGE_BASE),src)
	...
	$(CACHE_COMMAND) $(DOCKER_REPOSITORY):golang-$(RESTY_IMAGE_BASE)-$(RESTY_IMAGE_TAG)-$(DOCKER_KONG_SUFFIX) || \
	( $(MAKE) build-base ; \
	...
	-t $(DOCKER_REPOSITORY):golang-$(RESTY_IMAGE_BASE)-$(RESTY_IMAGE_TAG)-$(DOCKER_KONG_SUFFIX) . )
	-$(UPDATE_CACHE_COMMAND) $(DOCKER_REPOSITORY):golang-$(RESTY_IMAGE_BASE)-$(RESTY_IMAGE_TAG)-$(DOCKER_KONG_SUFFIX)
endif
...
ifneq ($(RESTY_IMAGE_BASE),src)
	...
	DOCKER_GO_BUILDER=$(DOCKER_REPOSITORY):golang-$(RESTY_IMAGE_BASE)-$(RESTY_IMAGE_TAG)-$(DOCKER_KONG_SUFFIX) \
	./test/run_tests.sh
endif
