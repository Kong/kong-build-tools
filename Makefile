...
ifneq ($(RESTY_IMAGE_BASE),src)
	...
	($(MAKE) build-openresty && \
	docker tag $(DOCKER_REPOSITORY):openresty-$(RESTY_IMAGE_BASE)-$(RESTY_IMAGE_TAG)-$(DOCKER_OPENRESTY_SUFFIX) $(DOCKER_REPOSITORY):test-$(DOCKER_OPENRESTY_SUFFIX))
	...
	--build-arg DOCKER_BASE_SUFFIX=$(DOCKER_BASE_SUFFIX) \
	--build-arg DOCKER_REPOSITORY=$(DOCKER_REPOSITORY) \
	-t $(DOCKER_REPOSITORY):test-$(KONG_SHA)-$(DOCKER_OPENRESTY_SUFFIX) .
	docker tag $(DOCKER_REPOSITORY):test-$(DOCKER_OPENRESTY_SUFFIX) $(DOCKER_REPOSITORY):test
	docker tag $(DOCKER_REPOSITORY):test-$(KONG_SHA)-$(DOCKER_OPENRESTY_SUFFIX) $(DOCKER_REPOSITORY):test-$(DOCKER_OPENRESTY_SUFFIX)
