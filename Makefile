...
KONG_OPENSSL_VERSION ?= `grep KONG_OPENSSL_VERSION $(KONG_SOURCE_LOCATION)/.requirements | awk -F"=" '{print $$2}'`
...
setup-build:
	.ci/setup_ci.sh
	$(info 'running build: ARCHITECTURE:     $(ARCHITECTURE)')
	$(info '               RESTY_IMAGE_BASE: $(RESTY_IMAGE_BASE)')
